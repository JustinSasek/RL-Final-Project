...
from rl.tennis.agentTrain import SarsaTennisAgent, EGreedyPolicy
import argparse
import sys
import os
import logging
...
class GameControl:
    ...
    def __init__(self, seed, out_dir, model_file, stats_file, capture_file, summary_file, monitor_file, 
        num_episode, view = False, model_save_freq = None):
        self.seed = seed
        ...
        self.behavior = LearnableTennisBehavior(seed = self.seed)
        ...
        self.agent = SarsaTennisAgent(self.behavior.cell_x, self.behavior.cell_y, 
            EGreedyPolicy(0.05, self.behavior.random), init_observed_state)
    ...
    def _do_train(self):
        ...
        while self.curr_episode < self.num_episode:
            ...
            if done == True:
                ...
                if (self._logger.isEnabledFor(logging.INFO)):
                    self._logger.info("Episode-End: {}, Time={} ms".format(self.curr_episode, episode_millis),
                        extra=self._LOG_TAG)
        ...
        final_qtable = "\nEnd Training Q-Table: {}".format(self.agent.tostr_qtable(self.agent.get_qtable()))
        ...
        if (self._logger.isEnabledFor(logging.INFO)):
            self._logger.info("Training completed for {} episodes, Time={} s".format(self.num_episode, training_time),
                extra=self._LOG_TAG)
            
        print("\n\nTraining completed at {} in {:.3f} s".format(strftime("%H:%M:%S", localtime()), training_time))
        print("Model stored at: {}\nSummary logged at: {}".format(self.model_file, self.summary_file))
    ...
    def _save_model(self, model_count):
        ...
        """
        file_path = os.path.sep.join([self.modeldir, "tn_{:06d}.npy".format(self.curr_episode)])
    ...
    def menu(self):
        ...
        while True:
            ...
            if action == 101:
                ...
                curr_qtable = "\nQ-Table at {}: {}".format(current_time, self.agent.tostr_qtable(self.agent.get_qtable()))
...
def to_path(args, purpose, out_dir):
    ...
    file_path = file_path if os.path.isabs(file_path) else os.path.sep.join([out_dir, file_path])
    return file_path
...
def main(argv):
    ...
    try:
        ...
        ap.add_argument("-m", "--modelFile", type=str, required=False, default=("tnmodel.npy"),
            help="Model file")
    ...
    game_control = GameControl(seed, out_dir, model_file, statistics_file, capture_file, summary_file, monitor_file,
        num_episode, view = show_animated_view, model_save_freq = model_save_freq)
