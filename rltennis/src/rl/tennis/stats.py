...
import os
import sys
from rl.tennis.discreteTennis import TennisStats, DiscreteTennis
...
def safe_div(numerator, denominator, on_zero = None, ret_zero = -1):
    ...
    """
    return (numerator/denominator) if denominator != 0 else (ret_zero if on_zero is None else (numerator/on_zero))

class GameStats(TennisStats):
    def __init__(self, stats_file):
        self.stats_fd = open(stats_file, "w")
        self.stats_fd.write("time,episode,event,winner,player_game,system_game,player_set,system_set,player_sets,system_sets,shots\n")
        self.count = 0
    ...
    def game_result(self, episode, event_type, winner, player_game, system_game, player_set, system_set, player_sets, system_sets, shots):
        ...
        buf = str(at_time) + \
            ...
            "," + str(event_type) + \
            "," + str(winner) + \
            "," + str(player_game) + "," + str(system_game) + \
            "," + str(player_set) + "," + str(system_set) + \
            "," + str(player_sets) + "," + str(system_sets) + \
            "," + str(shots) + "\n"
        self.stats_fd.write(buf)
        if self.count % 10 == 0:    # Flush periodically so that results don't get lost.
            self.flush()
...
class SummaryReport:
    ...
    def __init__(self):
        ...
        self._episode_start_totalshots = None
        self.episode_maxtime = -sys.maxsize - 1
        self.episode_max = None
        self.episode_mintime = sys.maxsize
        self.episode_min = None
        self.episode_maxshots = -sys.maxsize - 1
        self.episode_max_shots = None
        self.episode_minshots = sys.maxsize
        self.episode_min_shots = None
        
        self.episode_totalshots = 0
        self.episode_sumtime = 0
        self.episode_shots = 0
        self.episode_maxshots = 0
        self.episode_minshots = 0
        self.num_episode = 0
...
class ReportGenerator:
            ...
            with pd.read_csv(stats_fd, chunksize=chunksize) as reader:
                for df in reader:
                    for row in df.itertuples():
                        ...
                        if player_win:
                            self._report.player_game_win = self._report.player_game_win + 1
                            self._report.player_game_sumwinmargin = self._report.player_game_sumwinmargin + \
                                row.player_game - row.system_game + 1
                        else: 
                            self._report.system_game_win = self._report.system_game_win + 1
                            self._report.system_game_sumwinmargin = self._report.system_game_sumwinmargin + \
                                row.system_game - row.player_game + 1
                        
                        if row.event == 2:  # Set win
                            if player_win:
                                self._report.player_set_win = self._report.player_set_win + 1
                                self._report.player_set_sumwinmargin = self._report.player_set_sumwinmargin + \
                                    row.player_set - row.system_set
                            else: 
                                self._report.system_set_win = self._report.system_set_win + 1
                                self._report.system_set_sumwinmargin = self._report.system_set_sumwinmargin + \
                                    row.system_set - row.player_set
                        elif row.event == 3:  # Match win
                            if player_win:
                                self._report.player_match_win = self._report.player_match_win + 1
                                self._report.player_match_sumwinmargin = self._report.player_match_sumwinmargin + \
                                    row.player_sets - row.system_sets
                            else: 
                                self._report.system_match_win = self._report.system_match_win + 1
                                self._report.system_match_sumwinmargin = self._report.system_match_sumwinmargin + \
                                    row.system_sets - row.player_sets
    ...
    def _new_episode(self, row_number, row_episode, row, is_last = False):
        ...
        if self._report._curr_episode is not None:
            ...
            self._report.episode_sumtime = self._report.episode_sumtime + episode_duration
            self._report.num_episode = self._report.num_episode + 1 # Completed this many episodes in current item.
            
            episode_shots = self._report.total_shots - self._report._episode_start_totalshots
            self._report.episode_totalshots = self._report.episode_totalshots + episode_shots
            if episode_shots > self._report.episode_maxshots:
                self._report.episode_maxshots = episode_shots
                self._report.episode_max_shots = self._report._curr_episode
            
            if episode_shots < self._report.episode_minshots:
                self._report.episode_minshots = episode_shots
                self._report.episode_min_shots = self._report._curr_episode
        ...
        if not is_last:
            ...
            self._report._episode_start_totalshots = self._report.total_shots
    ...
    def _generate_whole_summary(self):
        ...
        for report in self._report_list:
            ...
            whole.player_game_sumwinmargin = whole.player_game_sumwinmargin = + report.player_game_sumwinmargin
            
            whole.system_game_win = whole.system_game_win + report.system_game_win
            whole.system_game_sumwinmargin = whole.system_game_sumwinmargin = + report.system_game_sumwinmargin
            
            whole.player_set_win = whole.player_set_win + report.player_set_win
            whole.player_set_sumwinmargin = whole.player_set_sumwinmargin = + report.player_set_sumwinmargin
            
            whole.system_set_win = whole.system_set_win + report.system_set_win
            whole.system_set_sumwinmargin = whole.system_set_sumwinmargin = + report.system_set_sumwinmargin
            
            whole.player_match_win = whole.player_match_win + report.player_match_win
            whole.player_match_sumwinmargin = whole.player_match_sumwinmargin = + report.player_match_sumwinmargin
            
            whole.system_match_win = whole.system_match_win + report.system_match_win
            whole.system_match_sumwinmargin = whole.system_match_sumwinmargin = + report.system_match_sumwinmargin
    ...
    def tostr_whole_summary(self):
        ...
        buf = buf + "Match Won:  Player={:<4.1f}% {:<8d} System={:<4.1f}% {:<8d}\n".\
            format(safe_div(rp.player_match_win, tot) * 100, rp.player_match_win, safe_div(rp.system_match_win, tot) * 100, rp.system_match_win)
        tot = rp.player_set_win + rp.system_set_win
        buf = buf + "Sets Won:   Player={:<4.1f}% {:<8d} System={:<4.1f}% {:<8d}\n".\
            format(safe_div(rp.player_set_win, tot) * 100, rp.player_set_win, safe_div(rp.system_set_win, tot) * 100, rp.system_set_win)
        tot = rp.player_game_win + rp.system_game_win
        buf = buf + "Games Won:  Player={:<4.1f}% {:<8d} System={:<4.1f}% {:<8d}\n".\
            format(safe_div(rp.player_game_win, tot) * 100, rp.player_game_win, safe_div(rp.system_game_win, tot) * 100, rp.system_game_win)
        buf = buf + "Episodes:   {:<8d} (Msec) Avg={:<8.2f} Min={:<8.2f}@{} Max={:<8.2f}@{}\n".\
            format(rp.num_episode, safe_div(rp.episode_sumtime, rp.num_episode), rp.episode_mintime, rp.episode_min, rp.episode_maxtime, rp.episode_max)
        buf = buf + "Margins:\n"
        buf = buf + "Game Won:   Player={:<8.2f} System={:<8.2f}\n".\
            format(safe_div(rp.player_game_sumwinmargin, rp.player_game_win), safe_div(rp.system_game_sumwinmargin, rp.system_game_win))
        buf = buf + "Set Won:    Player={:<8.2f} System={:<8.2f}\n".\
            format(safe_div(rp.player_set_sumwinmargin, rp.player_set_win), safe_div(rp.system_set_sumwinmargin, rp.system_set_win))
        buf = buf + "Match Won:  Player={:<8.2f} System={:<8.2f}\n".\
            format(safe_div(rp.player_game_sumwinmargin, rp.player_match_win), safe_div(rp.system_game_sumwinmargin, rp.system_match_win))
        return buf
    ...
    def _end_summary(self, stats_fd, last_row_index, last_row):
        ...
        if self._report is not None:
            if self._report.num_episode == 0 and self._report.total_shots == 0:
...
class GraphGenerator:
    ...
    def generate(self):
        ...
        """
        self._generate_wins()
        self._generate_episode_times()
        self._generate_episode_shots()
        self._generate_win_margins()
        
    def _generate_episode_times(self):
        """
        ...
        _, ax1 = plt.subplots()
        ax1.plot(self._episode_arr, avg_arr, color="blue", marker="o", linestyle="solid",
            linewidth=1, markersize=2, label="Average")
        ax1.plot(self._episode_arr, max_arr, color="red", marker="o", linestyle="solid",
            linewidth=1, markersize=2, label="Maximum")
        ax1.plot(self._episode_arr, min_arr, color="green", marker="o", linestyle="solid",
            linewidth=1, markersize=2, label="Minimum")
        
        ax1.legend()
        ax1.grid(visible=True, which="major", axis="both", color="#909090", linestyle="dotted", linewidth=0.8)
        
        ax1.set(xlabel="Episodes")
        ax1.set(ylabel="Time (msec)")
        ax1.set(title="Episode Times")
        
        fileName = os.path.sep.join([self._graph_dir, "episodeTimes.svg"])
        plt.savefig(fileName)
        
    def _generate_episode_shots(self):
        """
        Generate graph of report's last episode v/s the average, min and max time of episodes in each report.
        """
        avg_arr = np.empty(self._num_report, dtype=int)
        ...
        index = 0
        for report in self._report_list:
            avg_arr[index] = int(safe_div(report.episode_totalshots, report.num_episode))
            min_arr[index] = report.episode_minshots
            max_arr[index] = report.episode_maxshots
            index = index + 1
        ...
        ax1.set(xlabel="Episodes")
        ax1.set(ylabel="Shots")
        ax1.set(title="Episode Shots")
        
        fileName = os.path.sep.join([self._graph_dir, "episodeShots.svg"])
        plt.savefig(fileName)
    ...
    def _generate_wins(self):
        ...
        ax1.plot(self._episode_arr, player_arr, color="blue", marker="o", linestyle="solid",
            linewidth=1, markersize=2, label="Player")
        ax1.plot(self._episode_arr, system_arr, color="red", marker="o", linestyle="solid",
            linewidth=1, markersize=2, label="System")
        ...
        ax2.plot(self._episode_arr, player_arr, color="blue", marker="o", linestyle="solid",
            linewidth=1, markersize=2, label="Player")
        ax2.plot(self._episode_arr, system_arr, color="red", marker="o", linestyle="solid",
            linewidth=1, markersize=2, label="System")
        
        ax2.legend()
        ax2.grid(visible=True, which="major", axis="both", color="#909090", linestyle="dotted", linewidth=0.8)
        ...
        ax3.plot(self._episode_arr, player_arr, color="blue", marker="o", linestyle="solid",
            linewidth=1, markersize=2, label="Player")
        ax3.plot(self._episode_arr, system_arr, color="red", marker="o", linestyle="solid",
            linewidth=1, markersize=2, label="System")
        
        ax3.legend()
        ax3.grid(visible=True, which="major", axis="both", color="#909090", linestyle="dotted", linewidth=0.8)
    ...
    def _generate_win_margins(self):
        ...
        index = 0
        for report in self._report_list:
            player_arr[index] = safe_div(report.player_game_sumwinmargin, report.player_game_win)
            system_arr[index] = safe_div(report.system_game_sumwinmargin, report.system_game_win)
            index = index + 1
        ...
        index = 0
        for report in self._report_list:
            player_arr[index] = safe_div(report.player_set_sumwinmargin, report.player_set_win)
            system_arr[index] = safe_div(report.system_set_sumwinmargin, report.system_set_win)
            index = index + 1
        ...
        index = 0
        for report in self._report_list:
            player_arr[index] = safe_div(report.player_match_sumwinmargin, report.player_match_win)
            system_arr[index] = safe_div(report.system_match_sumwinmargin, report.system_match_win)
            index = index + 1
...
# Putting this as staticmethod inside QTableRenderer causes older version of python to complain and
# so this has been moved out for now.
...
class QTableRenderer:
    ...
    COL_MAP5 = create_custom_colormap("Yellows", 240, 220, 0, reverse_map=True) # No-Movement
    
    # Maps direction to color-map and whether direction points in positive direction.
    DIR_COLOR_MAP = {DiscreteTennis.DIR_FRONT: (COL_MAP1, True, COL_MAP1(1.0), " F"),
        ...
        DiscreteTennis.DIR_NONE: (COL_MAP5, True, (139/256, 128/256, 0.0, 1.0), "NO")}
    
    DIR_COLOR_ORDER = [DiscreteTennis.DIR_BACK, DiscreteTennis.DIR_FRONT, DiscreteTennis.DIR_LEFT, DiscreteTennis.DIR_RIGHT, \
        DiscreteTennis.DIR_BACKRIGHT, DiscreteTennis.DIR_FRONTLEFT, DiscreteTennis.DIR_BACKLEFT, DiscreteTennis.DIR_FRONTRIGHT,\
        DiscreteTennis.DIR_NONE]
    
    GCELLPOS_TO_DIR = np.array([
        [DiscreteTennis.DIR_FRONTRIGHT, DiscreteTennis.DIR_FRONT, DiscreteTennis.DIR_FRONTLEFT], # Row-0 Bottom-most
        [DiscreteTennis.DIR_RIGHT, DiscreteTennis.DIR_NONE, DiscreteTennis.DIR_LEFT],           # Row-1
        [DiscreteTennis.DIR_BACKRIGHT, DiscreteTennis.DIR_BACK, DiscreteTennis.DIR_BACKLEFT]   # Row-2 Top-most
        ])
    ...
    def plot_table(self, output_file, title = None):
        ...
        try:
            ...
            if (self._logger.isEnabledFor(logging.INFO)):
                self._logger.info("QTable Rendering qrange=[{}, {}], qtotal={}".format(self._qrange[0], self._qrange[1], self._qtotal),
                    extra=self._LOG_TAG)
    ...
    def _plot_key(self):
        ...
        for index in range(DiscreteTennis.DIR_MAX + 1):
            ...
            color = cm(0.5)
            kcell = patches.Rectangle((kcell_x, kcell_y), self._cell_x, self._cell_y, facecolor=color, edgecolor='black', lw=self._cell_line)
            self._ax.add_patch(kcell)
            
            rx, ry = kcell.get_xy()
            self._ax.annotate(dir_name, (rx + self._cell_valx, ry + self._cell_valy), color=full_color, weight='bold',
                fontsize=8, ha='center', va='center')
            
            title = self._title if self._title is not None else \
                "Q-Table, Mode={}".format("Next-Delta" if self._mode == QTableRenderer.MODE_NEXT_DELTA else "Max-QVal")
            self._ax.annotate(title, 
                (self._width/2, ry + 1.5 * self._cell_valy), weight='bold',
        ...
        for pt_row in range(3):
            ...
            for pt_col in range(3):
                ...
                if drift_dir != max_action_dir:
                    cmap, _, val_color, actdir_name = QTableRenderer.DIR_COLOR_MAP[max_action_dir]
                    qaction = max(self._qrange[0], min(max_action, self._qrange[1]))    # Clamp max_action between qrange[0] and qrange[1]
                    percent = 1.0 - (qaction - self._qrange[0])/self._qtotal # Max q gets lightest color.
                    color = cmap(percent)
                    
                    if debug_enabled:
                        debug_message = "Non At=({}, {}, {}): Action=({}, q={:.2f}, n={:.2f}), Color=({}, {:.3f}, {})".\
                            format(pt_x, pt_y, DiscreteTennis.DIR_NAME_MAP[drift_dir][1],  \
                            actdir_name, max_action, min_delta, cmap.name, percent, colors.rgb2hex(color, keep_alpha=True))
                else:
                    actdir_name = DiscreteTennis.DIR_NAME_MAP[max_action_dir][1]
                    if debug_enabled:
                        debug_message = "Opt At=({}, {}, {}): Action=({}, q={:.2f}, n={:.2f})".format(pt_x, pt_y, DiscreteTennis.DIR_NAME_MAP[drift_dir][1],  \
                            actdir_name, max_action, min_delta)

                gcell_x = pt_posx + (pt_col * self._cell_x)
                gcell_y = pt_posy + (pt_row * self._cell_y)
                
                if debug_enabled:
                    debug_message = debug_message + ", Layout=(pt=({}, {}), pos=({}, {}))".format(pt_row, pt_col, gcell_x, gcell_y)
                    
                gcell = patches.Rectangle((gcell_x, gcell_y), self._cell_x,
                    self._cell_y, facecolor=color, edgecolor='black', lw=self._cell_line)
                self._ax.add_patch(gcell)
                
                rx, ry = gcell.get_xy()
                gcell_val = min_delta if self._mode == self.MODE_NEXT_DELTA else max_action
                if pt_row == 1 and pt_col == 1:
                    self._ax.annotate("{},{}".format(pt_x, pt_y), (rx + self._cell_valx, ry + self._cell_valy), color='black', weight='bold',
                        fontsize=8, ha='center', va='center')
                    self._ax.annotate("{}: {:.2f}".format(actdir_name, gcell_val), (rx + self._cell_valx, ry + self._cell_diry), 
                        color=val_color, fontsize=6, ha='center', va='center')
                else:
                    if drift_dir != max_action_dir:
                        self._ax.annotate(actdir_name, (rx + self._cell_valx, ry + self._cell_diry), color=val_color, weight='bold',
                            fontsize=6, ha='center', va='center')
                    self._ax.annotate("{:.2f}".format(gcell_val), 
                        (rx + self._cell_valx, ry + self._cell_valy), color=val_color, fontsize=6, ha='center', va='center')
                row_list.append(gcell)
                
                if debug_enabled:
                    self._logger.debug(debug_message, extra=self._LOG_TAG)
            
        pt_cell = patches.Rectangle((pt_posx, pt_posy), (self._cell_x * 3),
            self._cell_y * 3, alpha=1, facecolor='none', edgecolor='black', lw=2)
    ...
    def show_color_map_tiles(cm, num_tiles):
        ...
        for col in range(num_tiles):
            ...
            cell = patches.Rectangle((pos_x, 0), cell_x, cell_y, facecolor=color, edgecolor='black', lw=1)
            ax.add_patch(cell)
            
            rx, ry = cell.get_xy()
            ax.annotate("{:.2f}".format(color_percent), (rx + cell_x/2, ry + cell_y/2), color='black', weight='bold',
                fontsize=6, ha='center', va='center')
...
def main(argv):
    ...
    qtable = np.random.rand(2, 3, DiscreteTennis.DIR_MAX + 1, DiscreteTennis.DIR_MAX + 1)
