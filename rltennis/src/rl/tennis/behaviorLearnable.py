...
import random
import logging
from rl.tennis.discreteTennis import DiscreteTennis, TennisBehavior, RenderEvent, SystemShot
...
class LearnableTennisBehavior(TennisBehavior):
    ...
    def __init__(self, config = {}):
        ...
        self.difficulty = config.get("difficulty", 0.95)
    ...
    def next_system_action(self, fire, serve, env):
        ...
        if fire or serve:
            ...
            if not serve:
                ...
                move_threshold = (1.0 - (self.difficulty))  # System wins slightly more against untrained player with 50% difficulty.
            ...
            if env._render_view:
                system_pt = env._position_to_point(env._to_render_position(DiscreteTennis.SYSTEM, system_pos))
                ...
                env._append_render_event(SystemShot(env._system, env._ball, system_pt, transit_start, shot_start, shot_end))
    ...
    def shot_target(self, hitter, hitter_at, receiver_at):
        ...
        """
        self.time_step += 1
        receiver = DiscreteTennis.PLAYER if hitter == DiscreteTennis.SYSTEM else DiscreteTennis.SYSTEM
        shot_at = None
        for _ in range(10):
            ...
            if seq is None:
                ...
                if (self._logger.isEnabledFor(logging.INFO)):
                    self._logger.info("{}New sequence {}".format(DiscreteTennis._LOG_LINE, seq),
                        extra=self._LOG_TAG)
            ...
            if shot_at is not None:
                ...
                if (self._logger.isEnabledFor(logging.INFO)):
                    self._logger.info("{}Seq History: {}".format(DiscreteTennis._LOG_LINE, seq._history),
                        extra=self._LOG_TAG)
...
class ShotSequenceFactory:
    ...
    def build_seq(self, seq_typename, behavior, receiver, start_at, nav_dir, upto = None):
        ...
        if builder is None:
            raise Exception("No registered sequence type with name={}".format(seq_typename))
        
        receiver_x = behavior.player_x if receiver == DiscreteTennis.PLAYER else behavior.system_x
        receiver_y = behavior.player_y if receiver == DiscreteTennis.PLAYER else behavior.system_y
        ...
        if nav_dir == DiscreteTennis.DIR_FRONT:
            ...
            upto = (start_at[0] - (receiver_x * total_steps), start_at[1] + (receiver_y * total_steps))
        ...
        return builder(behavior, receiver, start_at, nav_dir, step_delta, total_steps, upto)
    ...
    def rand_seq(self, behavior, receiver, start_at):
        ...
        while len(examine_dir) < DiscreteTennis.DIR_MAX:
            nav_dir = behavior.random.randrange(DiscreteTennis.DIR_MIN, DiscreteTennis.DIR_MAX)
...
class LinearProgression(ShotSequence):
    ...
    def _is_out_of_bound(self, pt_x, pt_y):
        ...
        """
        return ((pt_x < 0.0 or pt_x > 1.0 or pt_y < 0.0 or pt_y > 1.0) or
            (self._step_delta[0] > 0 and pt_x > self._upto[0]) or 
            (self._step_delta[0] < 0 and pt_x < self._upto[0]) or
            (self._step_delta[1] > 0 and pt_y > self._upto[1]) or 
            (self._step_delta[1] < 0 and pt_y < self._upto[1]))
    ...
    def __repr__(self):
        ...
        return("Receiver {}({}, {}): {}, {}".\
            format(entity, self._start_at[0], self._start_at[1], prog_name, dir_name))
...
class LinearRandom1xProgression(LinearProgression):
    ...
    def __init__(self, behavior, receiver, start_at, nav_dir, step_delta, estimated_steps, upto):
        super().__init__(receiver, start_at, nav_dir, step_delta, estimated_steps, upto)
    ...
    def _next_pos(self, time_step):
        ...
        reverse_multiplier = -1 if self._step_taken < int(self._estimated_steps * 0.8) and self._random.randint(0, 2) == 2 else 1 # Randoms: 0, 1, 2.
        ...
        if reverse_multiplier == -1 and (next_x < 0.0 or next_y < 0.0 or next_x > 1.0 or next_y > 1.0):
...
class LinearRandom1xEnd2xProgression(LinearProgression):
    ...
    def __init__(self, behavior, receiver, start_at, nav_dir, step_delta, estimated_steps, upto):
        ...
        # Random generator to be used.   
        self._random = behavior.random
        self._forward_jump = (self._upto[0] - 2 * self._step_delta[0], self._upto[1] - 2 * self._step_delta[1])
        self._backward_jump = (self._upto[0] - self._step_delta[0], self._upto[1] - self._step_delta[1])
    ...
    def _next_pos(self, time_step):
        ...
        if self._forward_jump is not None and self._forward and \
            self._shot_at[0] == self._forward_jump[0] and \
            self._shot_at[1] == self._forward_jump[1]:
            next_x = self._upto[0]
            next_y = self._upto[1]
            self._forward_jump = None
        elif self._backward_jump is not None and not self._forward and \
            self._shot_at[0] == self._backward_jump[0] and \
            self._shot_at[1] == self._backward_jump[1]:
