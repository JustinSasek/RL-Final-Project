...
class SimpleDiscreteTennisBehavior(TennisBehavior):
    ...
    def next_system_action(self, fire: bool, serve: bool, env: DiscreteTennis):
        ...
        if fire or serve:
            ...
            if abs(delta_x) > self.system_x:
                ...
                # Move to left.
                new_position_x = (
                    (system_pos[0] - self.system_x)
                    if delta_x < 0
                    else (system_pos[0] + self.system_x)
                )
                new_position_x = max(
                    0.0, min(new_position_x, 1.0)
                )  # Clamp between 0.0 and 1.0

            if abs(delta_y) > self.system_y:
                # Movement in y-direction is needed.
                new_position_y = (
                    (system_pos[1] - self.system_y)
                    if delta_y > 0
                    else (system_pos[1] + self.system_x)
                )
                new_position_y = max(
                    0.0, min(new_position_y, 1.0)
                )  # Clamp between 0.0 and 1.0
            ...
            if new_position_x != system_pos[0] or new_position_y != system_pos[1]:
                ...
                if env._render_view:
                    env._append_render_event(
                        SystemMove(
                            env._system,
                            None,
                            env._position_to_point(
                                env._to_render_position(
                                    DiscreteTennis.SYSTEM,
                ...
                if self._logger.isEnabledFor(logging.INFO):
                    self._logger.info(
                        env._log_state()
                        + "System Moves from {} to {}".format(
                            system_pos, (new_position_x, new_position_y)
                        ),
                        extra=self._LOG_TAG,
                    )
    ...
    def _shot_target_by2x_displace(
        ...
        """
        peer_2x = (
            2 * self.player_x if actor == DiscreteTennis.SYSTEM else 2 * self.system_x
        )
        ...
        if to_left is not None or to_right is not None:
            if to_left is not None and to_right is not None:
                use_left = bool(
                    self.random.getrandbits(1)
                )  # Shoot 2x randomly to left/right of peer's current position.
                return (
                    (to_left, peer_position[1], ball_owner)
                    if use_left
                    else (to_right, peer_position[1], ball_owner)
                )
                ...
                return (
                    ...
                    peer_position[1],
                    ball_owner,
                )  # Shoot 2x to left of peer's current position.
            elif to_right is not None:
                return (
                    to_right,
                    peer_position[1],
                    ball_owner,
                )  # Shoot 2x to right of peer's current position.
        return self._shot_target_by_stretch(
            actor, actor_at, peer_position, None, ball_owner
        )
    ...
    def shot_target(
        ...
        if (
            ...
            or actor == DiscreteTennis.SYSTEM
            and self.player_2x_displace
        ):
            # 2x displacement on - compute shot target by 2x displacement.
            return self._shot_target_by2x_displace(
                actor, hitter_at, peer_position, ball_owner
            )
        ...
        difficult_eval = self.random.random()
        is_difficult = difficult_eval < actor_difficulty
        is_very_difficult = difficult_eval < (
            actor_difficulty * self.difficulty_2x_factor
        )
        ...
        if is_very_difficult:
            ...
            if actor == DiscreteTennis.PLAYER:
                ...
                self.player_2x_displace = (
                    True  # Turn 2x displacement on until current game ends.
                )
                return self._shot_target_by2x_displace(
                    actor, hitter_at, peer_position, ball_owner
                )
